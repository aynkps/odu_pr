...
Функция ЗафиксироватьОшибкуExchange(Метод, ОтправлятьВТелеграмм = Истина, ПоказатьОшибку = Истина, Объект1С = "", ОшибкаДляТелеграмм = "", БазаУО = "", Версия = "", Данные = Неопределено, ДопИнформация = "")  Экспорт
	...
	Если ОтправлятьВТелеграмм = Истина Тогда
		ОтправитьСообщениеВТелеграмExchange(Метод, Объект1С, ОшибкаДляТелеграмм, БазаУО, Версия, Данные, ДопИнформация);
	КонецЕсли; 
	...
	Попытка
		...
		Иначе
			...
			Если Запись.Выбран() Тогда
				...
				//Список не обновляемых свойств:
				Если ЗначениеЗаполнено(Запись.ДополнительныеСвойстваОбъекта) И СтруктураНастроекОбмена.Свойство("ДополнительныеСвойстваОбъекта") Тогда
					
					ДопСвойстваОбъектаЗапись = ЗначениеИзСтрокиВнутр(Запись.ДополнительныеСвойстваОбъекта); 
					
					Если ТипЗнч(СтруктураНастроекОбмена.ДополнительныеСвойстваОбъекта) = Тип("Строка") Тогда
						ДопСвойстваОбъектаРегистрация = ЗначениеИзСтрокиВнутр(СтруктураНастроекОбмена.ДополнительныеСвойстваОбъекта);
					Иначе
						ДопСвойстваОбъектаРегистрация = СтруктураНастроекОбмена.ДополнительныеСвойстваОбъекта;
					КонецЕсли;
					
					Для каждого КлючЗначение Из ДопСвойстваОбъектаЗапись Цикл
						
						ИмяПараметра = КлючЗначение.Ключ;
						
						Если ИмяПараметра = "оду_ЭтоНовыйОбъект" ИЛИ ИмяПараметра = "ЭтоУдалениеОбъекта" Тогда
							ДопСвойстваОбъектаРегистрация.Вставить(ИмяПараметра, КлючЗначение.Значение);		
						КонецЕсли;
					
					КонецЦикла;   
					
					СтруктураНастроекОбмена.ДополнительныеСвойстваОбъекта = ДопСвойстваОбъектаРегистрация;	
...
Процедура ЗаполнитьЗаписьРегистрацииИзСтруктурыНастроекОбмена(Запись, СтруктураНастроекОбмена, ИменаПолейЗаполнения = "", ИменаПолейИсключений = "")
	...
	//TODO:  подумать
	//Если ИменаПолейЗаполнения = "" Тогда //Записи нет
...
//Из обработки.РегистрацияИзмененийДляОбменаДанными
...
Функция ВыполнитьВыгрузкуДанных(НастройкиТекущейБД, НомерВыполнения = 1, СсылкаУО = Неопределено, ЭтоРегламент = Ложь)	Экспорт
	...
	Попытка
		...
		Если Не ЗначениеЗаполнено(МассивСтруктурУчастниковОбмена)  Тогда
			Возврат Неопределено;		
		КонецЕсли; 
...
#Область  ВыполнениеОбменаПоПараметрам 


Функция СтруктураПараметровИзФормы() Экспорт 
	
	СтруктураПараметров = Новый Структура(); 
	СтруктураПараметров.Вставить("ИмяФайла");
	СтруктураПараметров.Вставить("ИмяФайлаИзСтроки");
	СтруктураПараметров.Вставить("ИмяФайлаПоВремени");
	СтруктураПараметров.Вставить("РежимОтладки"); 
	СтруктураПараметров.Вставить("ДополнительныеСвойстваОбъекта");
	СтруктураПараметров.Вставить("ТипXMLОбъекта");
	СтруктураПараметров.Вставить("УО");
	СтруктураПараметров.Вставить("РаспределятьПодчиненнымУО"); 
	СтруктураПараметров.Вставить("Конвертация_НастройкаИзПараметров");
	СтруктураПараметров.Вставить("ВыгрузитьЧЗФайл");
	СтруктураПараметров.Вставить("УзелПланаОбмена");
	СтруктураПараметров.Вставить("КаталогХраненияФайловОбработки"); 
	СтруктураПараметров.Вставить("ИДБазыИсточник");
	СтруктураПараметров.Вставить("ИДБазыПриемник");
	СтруктураПараметров.Вставить("ИДОбъекта"); 
	СтруктураПараметров.Вставить("ПутьКФайлуОбмена");
	
	Возврат СтруктураПараметров;
	
КонецФункции

Процедура ЗаполнитьПеременныеОбработки(СтруктураПараметров) 
	
	ИмяФайла = СтруктураПараметров.ИмяФайла; 
	ИмяФайлаИзСтроки = СтруктураПараметров.ИмяФайлаИзСтроки; 
	ИмяФайлаПоВремени = СтруктураПараметров.ИмяФайлаПоВремени;
	
	РежимОтладки = СтруктураПараметров.РежимОтладки;
	ДополнительныеСвойстваОбъекта = СтруктураПараметров.ДополнительныеСвойстваОбъекта; 
	ТипXMLОбъекта = СтруктураПараметров.ТипXMLОбъекта; 
	УО = СтруктураПараметров.УО; 
	РаспределятьПодчиненнымУО = СтруктураПараметров.РаспределятьПодчиненнымУО; 
	Конвертация_НастройкаИзПараметров = СтруктураПараметров.Конвертация_НастройкаИзПараметров; 
	ВыгрузитьЧЗФайл = СтруктураПараметров.ВыгрузитьЧЗФайл; 
	УзелПланаОбмена = СтруктураПараметров.УзелПланаОбмена; 
	КаталогХраненияФайловОбработки = СтруктураПараметров.КаталогХраненияФайловОбработки; 
	ИДБазыИсточник = СтруктураПараметров.ИДБазыИсточник; 
	ИДБазыПриемник = СтруктураПараметров.ИДБазыПриемник; 
	ИДОбъекта = СтруктураПараметров.ИДОбъекта; 
	ПутьКФайлуОбмена = СтруктураПараметров.ПутьКФайлуОбмена; 
	
КонецПроцедуры

Функция РезультатВыполненияОбменаПоПараметрамВФоне(СтруктураПараметров, ДанныеОбмена = Неопределено, Тек_ИДОбъекта = "", Тек_ТипXMLОбъекта = "", Тек_ИмяФайлаОбмена = "", ИмяПВД = "", НомерСообщения = "", СтруктураНастроекКонвертацииИзФормы = Неопределено) Экспорт
	
	ИмяПроцедуры = "оду_Общий.РезультатВыполненияОбменаПоПараметрам";

	МассивПараметров = Новый Массив;
	МассивПараметров.Добавить(СтруктураПараметров);
	МассивПараметров.Добавить(ДанныеОбмена);
	МассивПараметров.Добавить(Тек_ИДОбъекта);
	МассивПараметров.Добавить(Тек_ТипXMLОбъекта);
	МассивПараметров.Добавить(Тек_ИмяФайлаОбмена);
	МассивПараметров.Добавить(ИмяПВД);
	МассивПараметров.Добавить(НомерСообщения);
	МассивПараметров.Добавить(СтруктураНастроекКонвертацииИзФормы);
	МассивПараметров.Добавить(СтруктураПараметров);
	
	пр_Общий.ЗапуститьФоновоеВыполнение(ИмяПроцедуры, МассивПараметров);
	
КонецФункции
	
Функция РезультатВыполненияОбменаПоПараметрам(СтруктураПараметров, ДанныеОбмена = Неопределено, Тек_ИДОбъекта = "", Тек_ТипXMLОбъекта = "", Тек_ИмяФайлаОбмена = "", ИмяПВД = "", НомерСообщения = "", СтруктураНастроекКонвертацииИзФормы = Неопределено) Экспорт
	
	пр_Модуль = пр_Общий.ИсполняемыйМодуль("оду_УправлениеОбменом", пр_НастройкиПовтИсп.ТекущийПользователь()); 
	Если пр_Модуль = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли; 
	
	пр_Модуль.РезультатВыполненияОбменаПоПараметрам(СтруктураПараметров, ДанныеОбмена, Тек_ИДОбъекта, Тек_ТипXMLОбъекта, Тек_ИмяФайлаОбмена, ИмяПВД, НомерСообщения, СтруктураНастроекКонвертацииИзФормы);
...
Функция РезультатОчисткиЗаписейПослеЗагрузкиПоИмениФайла(МассивЗагруженныхФайловОбмена, ЭтоПриемник = Ложь, РаспределятьПодчиненнымУО = Ложь) Экспорт 
		...
		Если ВДЗ.Количество() > 0 Тогда
			...
			Если Лев(ВДЗ.ИДОбъекта, 4) = "UOD_" И ЭтоПриемник = Ложь И РаспределятьПодчиненнымУО = Ложь Тогда
				...
				Если ВыборкаУзла.Следующий() Тогда
					...
					НомерСообщенияОбмена = ВДЗ.НомерСообщенияОбмена;
					УзелОбменаСсылка = ВыборкаУзла.оду_УзелПланаОбмена;
					ПланыОбмена.УдалитьРегистрациюИзменений(УзелОбменаСсылка, НомерСообщенияОбмена);	
					
					Если НомерСообщенияОбмена >= 990 Тогда
						Данные = Новый Структура("НомерОтправленного, НомерПринятого", 0, 0);
						УстановитьПараметрыУзлаОбмена(УзелОбменаСсылка, Данные);
					КонецЕсли; 
