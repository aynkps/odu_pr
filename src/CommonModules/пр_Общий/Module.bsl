...
Функция СоответвиеПоЗапросу(РЗ, ИмяКлюча = "", ИмяЗначенияКлюча = "", ЗначениеПоУмолчанию = Неопределено, ИмяКлючаПоУмолчанию = "ПоУмолчанию")	Экспорт
	...
	Пока ВДЗ.Следующий() Цикл
		...
		Если ЭтоМассивСвойств = Истина Тогда
			СоответвиеПоЗапросу.Вставить(Ключ, СтруктураЗапроса); 
		Иначе	
...
Функция ЗначениеВСтрокуXML(Значение) Экспорт
	...
	Исключение
		...
		пр_КлиентСервер.ЗафиксироватьОшибкуDev(пр_Ошибка, "ЗначениеВСтрокуXML");
		Возврат Неопределено;
...
// Функция - ЗЗ - ЗаписатьЗапросВБазу
//
// Параметры:
//  Запрос	 - 	 - 
// 
// Возвращаемое значение:
//  РезультатЗаписи  -  Булево
//
Функция ЗЗ(Запрос) Экспорт 
	
	Результат = РезультатЗапросаВСтрокуXML(Запрос);
	
	Если Результат = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	СпрОбъект = Справочники.пр_Запросы.СоздатьЭлемент();
	СпрОбъект.Наименование = СтрШаблон("От_%1", Формат(ТекущаяДатаСеанса(), "ДФ=ггггММддччмм"));
	СпрОбъект.ЗапросВСтрокеXML = Результат;
	СпрОбъект.ОбменДанными.Загрузка = Истина;
	СпрОбъект.Записать();
	
КонецФункции 

Функция РезультатЗапросаВСтрокуXML(Запрос)  Экспорт
	...
	Результат = ЗначениеВСтрокуXML(Структура);
	Возврат Результат;
...
Функция УничтожитьВТ(МенеджерВременныхТаблиц, ИмяВременнойТаблицы) Экспорт 
	...
	Попытка
		...
		Запрос.Текст = СтрШаблон("  УНИЧТОЖИТЬ %1", ИмяВременнойТаблицы);
...
//
Процедура ДополнитьСтруктуру(СтруктураПриемник, СтруктураИсточник, СЗаменой = Неопределено, СписокИсключений = "") Экспорт
	...
	Для Каждого КлючИЗначение Из СтруктураИсточник Цикл
		...
		Если СтрНайти(СписокИсключений, Ключ) > 0 Тогда
			Продолжить;
		КонецЕсли; 
...
Процедура ДобавитьСписокСвойстВСтруктуру(СтруктураДанных)  Экспорт
	...
	пр_КлиентСервер.СписокСвойствВСтруктуру(СтруктураДанных);	
...
Функция ИсполнятьКод(ИмяМодуля, Метод, Пользователь = Неопределено)  Экспорт
	...
	Исключение
		пр_Ошибка = ОписаниеОшибки();
		пр_КлиентСервер.ЗафиксироватьОшибкуDev(пр_Ошибка, "ИсполнятьКод");
		Возврат Ложь;
	КонецПопытки;
	
КонецФункции 

Функция ТекстЗапросаДляОпределенияИсполняемогоКодаМетодаМодуля()
...
Функция ИсполнятьКодМетодаМодуля(ИмяМодуля, Метод, Пользователь = Неопределено)  Экспорт
	...
	Попытка
		...
		Запрос = Новый Запрос;
		Запрос.Текст = ТекстЗапросаДляОпределенияИсполняемогоКодаМетодаМодуля();
	...
	Исключение
		пр_Ошибка = ОписаниеОшибки();
		пр_КлиентСервер.ЗафиксироватьОшибкуDev(пр_Ошибка, "ИсполнятьКодМетодаМодуля");
		Возврат Ложь;
	КонецПопытки;
...
Функция ИсполняемыйМетодМодуля(ИмяМодуля, Метод = "", Пользователь = Неопределено)   Экспорт 
	...
	Исключение
		пр_Ошибка = ОписаниеОшибки();
		пр_КлиентСервер.ЗафиксироватьОшибкуDev(пр_Ошибка, "ИсполняемыйМетодМодуля");
		Возврат Неопределено;
	КонецПопытки;
...
Функция НастройкиИсполняемогоМетодаМодуля(ИмяМодуля, Метод = "", Пользователь = Неопределено)   Экспорт 
...
Функция ИсполняемыйМодуль(ИмяМодуля, Пользователь = Неопределено)   Экспорт 
	...
	Попытка
		...
		//Модуль может быть из МО,ММ справочника, документа, регистра
		Если СтрНайти(ИмяМодуля, ".") Тогда
			ИмяМодуляДляЗапроса = СтрЗаменить(ИмяМодуля, ".", "_");
		Иначе
			ИмяМодуляДляЗапроса = ИмяМодуля;
		КонецЕсли; 
		
		Запрос = Новый Запрос;
		Запрос.Текст = ТекстЗапросаДляОпределенияИсполняемогоМодуля();
		
		Запрос.УстановитьПараметр("ИмяМодуля", ИмяМодуляДляЗапроса);
		Запрос.УстановитьПараметр("Пользователь", Пользователь);
	...
	//Если не найдет попытка найти в текущей конфигурации
	Попытка
		ИсполняемыйМодуль = ИсполняемыйМодульОбщегоНазначения(ИмяМодуля);
		Возврат  ИсполняемыйМодуль;
	Исключение
		пр_Ошибка = ОписаниеОшибки();
		пр_КлиентСервер.ЗафиксироватьОшибкуDev(пр_Ошибка, "ИсполняемыйМодуль");
		Возврат Неопределено;
	КонецПопытки;
	
КонецФункции    

//TODO:  Доделать для МодулейОбъектов и МодулейМенеджеров
Функция ИсполняемыйМодульОбщегоНазначения(ИмяМодуля)
	
	ИсполняемыйМодуль = Неопределено;
	//МассивПутиМодуля = СтрРазделить(ИмяМодуля, ".");
	//Если МассивПутиМодуля.Количество() > 0 Тогда
	Если СтрНайти(ИмяМодуля, ".") Тогда
		ТекстПолученияИсполняемогоМодуля = СтрШаблон("ИсполняемыйМодуль = %1", ИмяМодуля);
		Выполнить(ТекстПолученияИсполняемогоМодуля);
		Возврат ИсполняемыйМодуль;
	ИначеЕсли Метаданные.ОбщиеМодули.Найти(ИмяМодуля) = Неопределено Тогда// ИЛИ СтрЧислоВхождений(ИмяМодуля, ".") <> 1 Тогда
		Возврат  Неопределено;
	КонецЕсли; 
	ИсполняемыйМодуль = ОбщегоНазначения.ОбщийМодуль(ИмяМодуля);
	Возврат  ИсполняемыйМодуль; 
...
//Перенаправление открия форм
Функция РезультатОбработкиПолученияФормы(ИмяОбъекта, ВидФормы, Параметры, ВыбраннаяФорма, СтандартнаяОбработка) Экспорт
	
	ТекущийПользователь = пр_НастройкиПовтИсп.ТекущийПользователь();
	
	ЭтаФормаВОтладке = пр_НастройкиПовтИсп.ИсполнятьКодМетодаМодуля(ИмяОбъекта, ВидФормы, ТекущийПользователь);
	
	Если ЭтаФормаВОтладке = Истина Тогда
		
		СтандартнаяОбработка = Ложь;
		
		ДополнительнаяИнформация = Новый Структура(); 	
		ДополнительнаяИнформация.Вставить("пр_ИмяОбъекта", ИмяОбъекта);
		ДополнительнаяИнформация.Вставить("пр_ВидФормы", ВидФормы);
		//ДополнительнаяИнформация.Вставить("пр_ТекущийПользователь", ТекущийПользователь);
		//пр_КлиентСервер.СписокСвойствВСтруктуру(ДополнительнаяИнформация);
		
		Если Параметры = Неопределено  Тогда
			Параметры = Новый Структура(); 	
		КонецЕсли; 
		
		Параметры.Вставить("пр_ДополнительнаяИнформация", ДополнительнаяИнформация);
		
		ВыбраннаяФорма = "ОбщаяФорма.От_ФормаОтладки";
		
	КонецЕсли; 
	
	Возврат ЭтаФормаВОтладке;
...
// Функция - Тип XMLИз идентификатора объекта метаданных
//
// Параметры:
//  Объект	 - СправочникСсылка.ИдентификаторыОбъектовМетаданных
// 
// Возвращаемое значение:
//  ТипXML - Строка ЗначениеПустойСсылки 
//
Функция ТипXMLПустогоЗначенияИзИдентификатораОбъектаМетаданных(Знач Объект)	Экспорт
	...
	Попытка
		...
		Если ЗначениеЗаполнено(Объект) Тогда    
			...
			Если ТипЗнч(Объект) = Тип("СправочникСсылка.ИдентификаторыОбъектовМетаданных") Тогда
				...
				Данные = ЗначениеРеквизитаПоСсылке(Объект, "ЗначениеПустойСсылки");
				Если Данные <> Неопределено Тогда
					ТипXML = ИмяТипаXMLПоСсылке(Данные);
				Иначе
					МассивИмени	= СтрРазделить(Объект.ПолноеИмя, ".");  
	...
	Исключение
		пр_Ошибка = ОписаниеОшибки();
		пр_Общий.ОтправитьСообщениеВТелеграмDev("ТипXMLПустогоЗначенияИзИдентификатораОбъектаМетаданных", Объект, пр_Ошибка);
	КонецПопытки;
...
//
Функция СохранитьТЗ(ТЗ, ПолныйПутьКФайлу = "", ИмяФайла = "", ПутьККаталогу = "", ТипХранения = Неопределено, СоздаватьКаталогЕслиНеСуществует = Истина) Экспорт
	
	Попытка
		
		ПолныйПутьКФайлу = ПолныйПутьКФайлу(ПолныйПутьКФайлу, ИмяФайла, ПутьККаталогу, ТипХранения, СоздаватьКаталогЕслиНеСуществует);
...
Функция ПолныйПутьКФайлу(ПолныйПутьКФайлу = "", ИмяФайла = "", ПутьККаталогу = "", ТипХранения = "json", СоздаватьКаталогЕслиНеСуществует = Ложь) Экспорт
	...
	//Если ЗначениеЗаполнено(ТипХранения) Тогда
		ПутьКФайлу = СтрШаблон("%1.%2", ПутьКФайлу, СтрЗаменить(Строка(ТипХранения), ".", ""));		
	//Иначе //По умолчанию 
...
// Получение значения из строки, для вызова из клиентских методов
//
// Параметры:
//  СтрокаДанных	 - Произвольные данные	
// 
// Возвращаемое значение:
//  Строка	 - Строка	 -  Полученная из метода ЗначениеВСтрокуВнутр
//
Функция ЗначениеВСтроку(СтрокаДанных) Экспорт 
	
	Попытка
		Данные = ЗначениеВСтрокуВнутр(СтрокаДанных);
	Исключение
		Ошибка = ОписаниеОшибки();
		Возврат Неопределено;
	КонецПопытки;
...
// 
// Возвращаемое значение:
//   Произвольные данные, Неопределено 
//
Функция ЗначениеИзСтроки(СтрокаДанных) Экспорт 
	
	Попытка
		Данные = ЗначениеИзСтрокиВнутр(СтрокаДанных);
	Исключение
		Ошибка = ОписаниеОшибки();
		Возврат Неопределено;
	КонецПопытки;
...
Функция ЗапуститьФоновоеВыполнение(ИмяМетода, МассивПараметров = Неопределено, ИДФоновогоЗадания = "", НаименованиеФоновогоЗадания = "", ИнтерактивныйВызов = Ложь) Экспорт
	...
	Исключение
		пр_Ошибка = ОписаниеОшибки();
		пр_КлиентСервер.ЗафиксироватьОшибкуDev(пр_Ошибка, "ЗапуститьФоновоеВыполнение");
		Возврат Неопределено;
	КонецПопытки;
...
Процедура СоздатьНаФормеРеквизитаТЧ(ЭтаФорма, ИмяТЧ = "", ИмяПоля, Заголовок, Тип, ИмяГруппы, ПоместитьПеред = "", ВидПоляФормы = Неопределено, Действие = "", ДобавитьРеквизитФормы = Ложь)
	
	//Заголовок = СтрЗаменить(ИмяПоля, "аян_", "*");
	Если ДобавитьРеквизитФормы = Истина Тогда
		пр_Общий.ДобавитьРеквизитФормы(ЭтаФорма, ИмяПоля, Тип, Заголовок, ИмяТЧ,, ИмяТЧ, ИмяТЧ + ИмяПоля);
	КонецЕсли; 
	
	СтруктураРеквизита = Новый Структура(); 
	Если ИмяТЧ = "" Тогда
		СтруктураРеквизита.Вставить("ПутьКДанным", ИмяПоля); //"ТабличнаяЧасть.аян_Добавлен");
	Иначе
		СтруктураРеквизита.Вставить("ПутьКДанным", СтрШаблон("%1.%2", ИмяТЧ, ИмяПоля)); //"ТабличнаяЧасть.аян_Добавлен");
	КонецЕсли;
	
	СтруктураРеквизита.Вставить("ИмяГруппы", ИмяГруппы);//"ТабличнаяЧасть");
	
	Если ЗначениеЗаполнено(ПоместитьПеред)  Тогда
		СтруктураРеквизита.Вставить("ПоместитьПеред", ПоместитьПеред);//"ТабличнаяЧастьХарактеристика");
	КонецЕсли;
	
	Если ВидПоляФормы <> Неопределено Тогда
		СтруктураРеквизита.Вставить("ВидПоляФормы", ВидПоляФормы);//ВидПоляФормы.ПолеФлажка);
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(Действие)  Тогда
		...
		СтруктураРеквизита.Вставить("ОбработкаСобытий", аян_Событие);
	КонецЕсли;
	
	аян_МассивРеквизитов = Новый Массив;
	аян_МассивРеквизитов.Добавить(Новый Структура("Имя, Значение", ИмяПоля, СтруктураРеквизита));
...
Функция ОтправитьСообщениеВТелеграмDev(Метод, Объект1С = Неопределено, ОшибкаДляТелеграмм = "", Версия = "", Данные = Неопределено, ДопИнформация = Неопределено)  Экспорт
	
	ИмяПроцедуры = "пр_Общий.ОтправитьСообщениеВТелеграмDevДалее";
	МассивПараметров = Новый Массив;
	...
	пр_Общий.ЗапуститьФоновоеВыполнение(ИмяПроцедуры, МассивПараметров, , "ОтправитьСообщениеВТелеграмDevДалее"); 
	
КонецФункции

Функция ОтправитьСообщениеВТелеграмDevДалее(Метод, Объект1С = Неопределено, ОшибкаДляТелеграмм = "", Версия = "", Данные = Неопределено, ДопИнформация = Неопределено)  Экспорт
	...
	Попытка
		...
		ЗаполнитьТекстДанныхТелеграмм(text, Версия, "Версия");
		
		ЗаполнитьТекстДанныхТелеграмм(text, Данные, "Данные");
		ЗаполнитьТекстДанныхТелеграмм(text, ДопИнформация, "ДопИнформация");
		
		//Описание Ошибки
		ЗаполнитьТекстДанныхТелеграмм(text, ОшибкаДляТелеграмм, "Ошибка");
...
Процедура ЗаполнитьТекстДанныхТелеграмм(text, Данные, ИмяПараметра)
	
	Если ЗначениеЗаполнено(Данные) Тогда            
		ТекстИзЗначенияОбъекта = ТекстИзЗначенияОбъекта(Данные);
		ТекстДопИнформация = СтрШаблон("%1 :%2%3%4", ИмяПараметра, Символы.ПС, ТекстИзЗначенияОбъекта, Символы.ПС);
		text = text + ТекстДопИнформация;
	КонецЕсли; 
...
Функция ТекстИзЗначенияОбъекта(ЗначениеОбъекта = Неопределено)  Экспорт 
	
	ДопИнформацияТекст = "";
	
	Если ЗначениеЗаполнено(ЗначениеОбъекта) Тогда
		Если ТипЗнч(ЗначениеОбъекта) = пр_НастройкиПовтИсп.ТипСтруктура() Тогда
			...
			КонецЦикла;
		ИначеЕсли ТипЗнч(ЗначениеОбъекта) = пр_НастройкиПовтИсп.ТипСтрока() Тогда
			ДопИнформацияТекст = ЗначениеОбъекта;	
		Иначе
			ДопИнформацияТекст = пр_Общий.ЗначениеВСтроку(ЗначениеОбъекта);	
		КонецЕсли; 
		
	КонецЕсли;
	
	Возврат ДопИнформацияТекст;
...
//pdf
Функция sendDocument(chat_id, token, ИмяФайлаПолное, document = "", caption = Неопределено, disable_notification = Неопределено, reply_to_message_id = 0, reply_markup = Неопределено) Экспорт
	...
	Параметры.Вставить("chat_id", ЧислоВСтроку(chat_id));
	Параметры.Вставить("document", document);
	Параметры.Вставить("caption", caption);
	Параметры.Вставить("disable_notification", ФорматироватьБулево(disable_notification));
	Параметры.Вставить("reply_to_message_id", reply_to_message_id);
	Параметры.Вставить("reply_markup", reply_markup);
	
	Данные = СформироватьДанные(document, ИмяФайлаПолное, "document", "application/pdf");
	
	Результат = ОтправитьHTTPЗапросТелеграм(token, "sendDocument", Параметры, Данные);
...
Функция СформироватьДанные(Параметр = Неопределено, ИмяФайлаПолное, Name, ContentType)
	...
	Если НЕ ЗначениеЗаполнено(Параметр) Тогда
		...
		Данные.Вставить("Content-Type", ContentType);
