...
Функция ЗаписатьВПротоколВыполнения(Код="", СтруктураЗаписи=Неопределено, ВзвестиФлагОшибок=Истина, 
	...
	Если ВзвестиФлагОшибок Тогда
		...
		// ▼ ayan#мпп#bitrix2289 Быстрый и эффективный обмен КирилловПС  2024.11.20 ▼ Начало
		оду_Общий.ОтправитьСообщениеВТелеграмExchange("ЗаписатьВПротоколВыполнения", , ИтоговаяСтрокаДляЗаписи,,, СтруктураЗаписи);
		// ▲ ayan#мпп#bitrix2289 Быстрый и эффективный обмен КирилловПС  2024.11.20 ▲ Конец 
...
Процедура ВыгрузкаОбъектаВыборки(Объект, Правило, Свойства=Неопределено, ВходящиеДанные=Неопределено, ВыборкаДляВыгрузкиДанных = Неопределено)
	...
	// ▼ ayan#мпп#bitrix2289 Быстрый и эффективный обмен КирилловПС  2024.11.20 ▼ Начало
	Если ФлагОшибки Тогда
		оду_ЗафиксироватьОшибку(Объект, "Не удалось выгрузить объект");
	КонецЕсли; 
	// ▲ ayan#мпп#bitrix2289 Быстрый и эффективный обмен КирилловПС  2024.11.20 ▲ Конец 
