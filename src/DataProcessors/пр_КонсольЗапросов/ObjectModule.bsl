		...
		Для каждого Поле Из Строка.Поля Цикл
			...
			Иначе
				...
				Если Позиция > 1 Тогда 
					...
					// ▼ ayan#bitrix#5 Не может найти таблицу БД  КирилловПС  2024.09.27  ▼ Начало      
					//ИмяТаблицы =  СоответствиеБД.Найти(ИмяОбъекта, "Ключ").Значение;
					пр_РезультатПоиска = СоответствиеБД.Найти(ИмяОбъекта, "Ключ");
					Если пр_РезультатПоиска = Неопределено Тогда
						Продолжить;
					КонецЕсли;     
					ИмяТаблицы = пр_РезультатПоиска.Значение;
					// ▲ ayan#bitrix#5 Не может найти таблицу БД  КирилловПС  2024.09.27  ▲ Конец 
					НоваяСтрока = СоответствиеБДПоля.Добавить();
					НоваяСтрока.Ключ = Поле.ИмяПоляХранения;
