...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	//TODO:  Ошибка старых версий
	Если Не ЗначениеЗаполнено(ЗавершениеРаботы)  Тогда
...
Процедура ОткрытьКонструкторЗапроса(Команда)
	...
	#Иначе
		...
		//TODO:  
		Если НЕ ЗначениеЗаполнено(ТекстЗапросаВФорме)  Тогда
...
&НаКлиенте
Процедура пр_Сохранить(Команда)
	
	ТекЭлемент = Элементы.пр_Запросы.ТекущиеДанные;
	
	Если ТекЭлемент = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Если ТекЭлемент.ЭтоГруппа = Истина Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = "Выберете не группу";
		Сообщение.Сообщить(); 
		Возврат;
	КонецЕсли;
	
	Результат = пр_СохранитьНаСервере(ТекЭлемент.Ссылка);
...
&НаСервере
Функция пр_СохранитьНаСервере(пр_СсылкаЗапроса)
	
	Если НЕ ЗначениеЗаполнено(ИдентификаторТекущегоЗапроса)  Тогда
		Возврат Ложь;	
	КонецЕсли;
	
	пр_НайденныеЗапросы = Объект.Запросы.НайтиСтроки(Новый Структура("Идентификатор", ИдентификаторТекущегоЗапроса));

	Если пр_НайденныеЗапросы.Количество() = 0 Тогда
		Возврат Ложь;	
	КонецЕсли; 
	
	пр_текЗапрос = пр_НайденныеЗапросы[0];
	
	пр_НайденныеПараметры = Объект.Параметры.НайтиСтроки(Новый Структура("ИдентификаторЗапроса", ИдентификаторТекущегоЗапроса));
	
	пр_ПараметрыЗапроса	= Неопределено;
	
	Если пр_НайденныеПараметры.Количество() > 0 Тогда
		
		пр_ПараметрыЗапроса	= Новый Массив;
		Для каждого Эл Из пр_НайденныеПараметры Цикл
			пр_СтруктураПараметраЗапроса = пр_СтруктураПараметраЗапроса();
			ЗаполнитьЗначенияСвойств(пр_СтруктураПараметраЗапроса, Эл); 
		    пр_ПараметрыЗапроса.Добавить(пр_СтруктураПараметраЗапроса);
		КонецЦикла; 
		
	КонецЕсли; 
	
	пр_НайденныеМВТ = Объект.пр_МВТ.НайтиСтроки(Новый Структура("ИдентификаторЗапроса", ИдентификаторТекущегоЗапроса));
	
	пр_МВТ = Неопределено;
	
	Если пр_НайденныеМВТ.Количество() > 0 Тогда
		
		пр_МВТ	= Новый Массив;
		Для каждого Эл Из пр_НайденныеМВТ Цикл
			
			СтМВТ = Новый Структура("ПолноеИмя, Значение, КолВо");
			ЗаполнитьЗначенияСвойств(СтМВТ, Эл); 
			пр_МВТ.Добавить(СтМВТ);
			
		КонецЦикла; 
		
	КонецЕсли; 
	
	РезультатЗаписи = пр_РезультатЗаписиЗапроса(пр_СсылкаЗапроса, пр_текЗапрос.Текст, пр_ПараметрыЗапроса, пр_МВТ);
...
&НаСервере
Функция пр_СтруктураПараметраЗапроса()
	
	пр_СтруктураПараметра = Новый Структура();
	пр_СтруктураПараметра.Вставить("Значение");
	пр_СтруктураПараметра.Вставить("ЗначениеВФорме");
	пр_СтруктураПараметра.Вставить("Идентификатор");
	пр_СтруктураПараметра.Вставить("Имя");
	пр_СтруктураПараметра.Вставить("Тип");
	пр_СтруктураПараметра.Вставить("ТипВФорме");
	
	Возврат пр_СтруктураПараметра;
	
КонецФункции 

&НаСервере
Функция пр_СтруктураМВТЗапроса()
...
&НаСервере
Функция пр_РезультатЗаписиЗапроса(СсылкаЗапроса, ТекстЗапроса, ПараметрыЗапроса, МенеджерВременныхТаблиц)
	
	Попытка
		
		пр_ОбъектЗапроса = СсылкаЗапроса.ПолучитьОбъект();
		пр_ОбъектЗапроса.Идентификатор = пр_Общий.ИД_Ссылки(СсылкаЗапроса);
		пр_ОбъектЗапроса.ТекстЗапроса = ТекстЗапроса;
		пр_ОбъектЗапроса.ПараметрыЗапроса = ЗначениеВСтрокуВнутр(ПараметрыЗапроса);
		пр_ОбъектЗапроса.МенеджерВременныхТаблиц = ЗначениеВСтрокуВнутр(МенеджерВременныхТаблиц);
		
		пр_ОбъектЗапроса.ОбменДанными.Загрузка = Истина;
		пр_ОбъектЗапроса.Записать();
		
	Исключение
		пр_Ошибка = ОписаниеОшибки();
		пр_КлиентСервер.ЗафиксироватьОшибкуDev(пр_Ошибка, "пр_РезультатЗаписиЗапроса");
		Возврат Ложь;
	КонецПопытки;
...
&НаКлиенте
Процедура пр_ЗапросыПриАктивизацииСтроки(Элемент)
	
	//пр_СсылкаТекущегоЗапроса = пр_СсылкаТекущегоЗапроса();
	//
	//Если пр_СсылкаТекущегоЗапроса = Неопределено Тогда
	//	Возврат;
	//КонецЕсли;
	//
	//пр_ЗаполнитьДанныеФормыПоСсылкеЗапроса(пр_СсылкаТекущегоЗапроса);

КонецПроцедуры

&НаСервере
Процедура пр_ЗаполнитьДанныеФормыПоСсылкеЗапроса(пр_СсылкаЗапроса)
	
	Объект.Запросы.Очистить();
	Объект.Параметры.Очистить();
	Объект.пр_МВТ.Очистить();
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	пр_Запросы.Наименование КАК Имя,
		|	пр_Запросы.ТекстЗапроса КАК Текст,
		|	пр_Запросы.ПараметрыЗапроса КАК Параметры,
		|	пр_Запросы.МенеджерВременныхТаблиц КАК МВТ,
		|	""Идентификатор"" КАК Идентификатор
		|ИЗ
		|	Справочник.пр_Запросы КАК пр_Запросы
		|ГДЕ
		|	пр_Запросы.Ссылка = &Ссылка";
	
	Запрос.УстановитьПараметр("Ссылка", пр_СсылкаЗапроса);
	...
	Пока ВДЗ.Следующий() Цикл
		...
		Нстр = Объект.Запросы.Добавить();
		ЗаполнитьЗначенияСвойств(Нстр, ВДЗ); 
		//Нстр.Идентификатор = пр_Общий.ИД_Ссылки(пр_СсылкаЗапроса);	
		Нстр.Идентификатор = ИдентификаторТекущегоЗапроса;
		//Нстр.ИсходныйНомерСтроки = 1;	
		//Нстр.Имя = ИдентификаторТекущегоЗапроса;	
		
		//ИдентификаторТекущегоЗапроса = пр_СсылкаЗапроса;
		Если ЗначениеЗаполнено(ВДЗ.Параметры)  Тогда
			пр_ПараметрыЗапроса = ЗначениеИзСтрокиВнутр(ВДЗ.Параметры);
			Если ЗначениеЗаполнено(пр_ПараметрыЗапроса)  Тогда
				
				Для каждого СтруктураПараметров Из пр_ПараметрыЗапроса Цикл
					Нстр = Объект.Параметры.Добавить();
					ЗаполнитьЗначенияСвойств(Нстр, СтруктураПараметров);
					//Нстр.Значение = ЗначениеИзСтрокиВнутр(СтруктураПараметров.Значение);
					Нстр.ИдентификаторЗапроса = ИдентификаторТекущегоЗапроса;
				КонецЦикла; 
				
			КонецЕсли; 
		КонецЕсли; 
		
		Если ЗначениеЗаполнено(ВДЗ.МВТ)  Тогда
			пр_МассивМВТ = ЗначениеИзСтрокиВнутр(ВДЗ.МВТ);
			Если ЗначениеЗаполнено(пр_МассивМВТ)  Тогда
				Для каждого СтруктураМВТ Из пр_МассивМВТ Цикл
					Нстр = Объект.пр_МВТ.Добавить();
					ЗаполнитьЗначенияСвойств(Нстр, СтруктураМВТ);
					Нстр.Идентификатор = ИдентификаторТекущегоЗапроса;
					Нстр.ИдентификаторЗапроса = ИдентификаторТекущегоЗапроса;
				КонецЦикла; 
			КонецЕсли; 
...
&НаКлиенте
Функция пр_СсылкаТекущегоЗапроса()
	...
	Если Не ЗначениеЗаполнено(ТекЭлемент.Ссылка) ИЛИ ТекЭлемент.ЭтоГруппа = Истина Тогда
		Возврат Неопределено;
	КонецЕсли;
...
&НаКлиенте
Процедура пр_ЗапросыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	//TODO:  подумать, если запрос не менялся
	ИД_Ссылки = пр_Общий.ИД_Ссылки(пр_СсылкаТекущегоЗапроса);
	Если ИД_Ссылки <> пр_Общий.ИД_Ссылки(ИдентификаторТекущегоЗапроса) Тогда
		ИдентификаторТекущегоЗапроса = Новый УникальныйИдентификатор(ИД_Ссылки);	
		пр_ЗаполнитьДанныеФормыПоСсылкеЗапроса(пр_СсылкаТекущегоЗапроса);
		ОбновитьФормуКлиент();
	КонецЕсли; 
...
&НаКлиенте
Процедура пр_ЗаписатьЗапрос(Команда)
	Запрос_ВСтрокуНаСервере();
КонецПроцедуры

&НаСервере
Процедура Запрос_ВСтрокуНаСервере()
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	Номенклатура.Ссылка.Код = ""00-00003070""";
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ВТ_Данные.Ссылка,
		|	ВТ_Данные.ПометкаУдаления
		|ИЗ
		|	ВТ_Данные КАК ВТ_Данные";
	
	РезультатЗапроса = Запрос.Выполнить();
	//пр_Общий.ЗапросВСтрокуXML(Запрос, Истина, "\\pivo.local\Resource\Личные\Кириллов ПС\Буфер\ЗапросВСтрокуXML.txt");
	ЗЗ(Запрос);
