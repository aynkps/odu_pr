					...
					Пока ВДЗ.Следующий() Цикл
						...
						Если (Сч = КолОбъектовВОбмене ИЛИ КолТекЭл >= КолОбъектовОбмена) //И ИмяТипаXML_До <> Неопределено 
							...
							СтрокаОтветаОбмена = СтрокаОтветаОбменаВыгрузкуДанныхУОДКД2(ТаблицаСостоянийОбмена, Этап, КолЦиклов, ИмяФайлаОбмена, ИмяТипаXML, МассивСсылок1С, 
													СоответвиеПараметровВыгрузки, ДополнительныеСвойстваОбъекта, ОбработкаОбменаДанных, ПараметрыКонвертации, ВДЗ.ИДБазыИсточник = ВДЗ.ИДБазыПриемник);
							Сч = 0;	
...
//TODO:  доработать
Функция СтрокаОтветаОбменаВыгрузкуДанныхУОДКД2(ТаблицаСостоянийОбмена, Этап, КолЦиклов, ИмяФайлаОбмена, ИмяТипаXML,  МассивСсылок1С, СоответвиеПараметровВыгрузки, ДополнительныеСвойстваОбъекта, ОбработкаОбменаДанных, ПараметрыКонвертации, РаспределитьВсемПодчиненнымУО = Ложь)
	...
	СтрокаОтветаОбмена = ВыполнитьВыгрузкуДанныхУОДКД2(ТаблицаСостоянийОбмена, СоответвиеПараметровВыгрузки, ИмяФайлаОбменаДалее, ОбработкаОбменаДанных, ПараметрыКонвертации, РаспределитьВсемПодчиненнымУО);
...
Функция ВыполнитьВыгрузкуДанныхУОДКД2(ТаблицаСостоянийОбмена = Неопределено, СоответвиеПараметровВыгрузки = Неопределено, 
										ПутьКФайлуОбмена, ОбработкаОбменаДанных = Неопределено, ПараметрыКонвертации, РаспределитьВсемПодчиненнымУО = Ложь)    Экспорт 
	...
	Если ТаблицаСостоянийОбмена <> Неопределено Тогда
		...
		стр = ТаблицаСостоянийОбмена[0];
		ЗаполнитьЗначенияСвойств(оду_СтруктураУО, стр); 
		
		ЗначениеПараметра = ЗначениеВСтрокуВнутр(оду_СтруктураУО);
		//ОбработкаОбменаДанных.Параметры.Вставить("оду_СтруктураУО", ЗначениеВСтрокуВнутр(оду_СтруктураУО));
	...
	Если ПараметрыКонвертации.ИмяПВД = "ВыгрузкаДанныхИзПланаОбмена" Тогда
		...
		ОбработкаОбменаДанных.Данные.Вставить("оду_УзелПланаОбмена", ПараметрыКонвертации.оду_УзелПланаОбмена); 	
	КонецЕсли; 
	...
	Если ТаблицаСостоянийОбмена <> Неопределено Тогда
		ЗаписатьСостоянияПослеВыгрузкиОбъектов(СтруктураДанных.ТаблицаСостоянийОбмена, Результат, РаспределитьВсемПодчиненнымУО);
	КонецЕсли; 
...
Функция ЗаписатьСостоянияПослеВыгрузкиОбъектов(ТаблицаСостоянийОбмена, ВыгруженоБезОшибок, РаспределитьВсемПодчиненнымУО = Ложь)
	...
	Попытка
		...
		Если РаспределитьВсемПодчиненнымУО = Истина И ВыгруженоБезОшибок = Истина Тогда    
			МассивЗагруженныхФайловОбмена = Неопределено; 
			РезультатРаспределения = РезультатРаспределенияПоПодчиненнымУО(ТаблицаДляЗаписи, МассивЗагруженныхФайловОбмена);			
		...
		Если  РезультатРаспределения = Истина И ВыгруженоБезОшибок = Истина Тогда   
			РезультатОчисткиЗаписей = РезультатОчисткиЗаписейПослеЗагрузкиПоИмениФайла(МассивЗагруженныхФайловОбмена, , РаспределитьВсемПодчиненнымУО);
		КонецЕсли;
...
Функция РезультатРаспределенияПоПодчиненнымУО(ТЗ, МассивЗагруженныхФайловОбмена = Неопределено)   Экспорт
...
Функция РезультатОчисткиЗаписейПослеЗагрузкиПоИмениФайла(МассивЗагруженныхФайловОбмена, ЭтоПриемник = Ложь, РаспределитьВсемПодчиненнымУО = Ложь)
	...
	Попытка
		...
		Если ЭтоПриемник = Истина ИЛИ РаспределитьВсемПодчиненнымУО = Истина Тогда
...
Функция РезультатВыполненияПроизвольногоЗапроса(СтруктураЗапроса, ОбработкаЗапроса = Неопределено, ЭтоЗапрос = Ложь)    Экспорт
	...
	Если СтруктураЗапроса.ПодготовитьДанные = Ложь И ЭтоЗапрос = Ложь Тогда
