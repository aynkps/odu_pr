...
Функция ИнформацияОСистеме() Экспорт
	...
	ИнформацияОСистеме.Вставить("ВерсияПриложения", СисИнфо.ВерсияПриложения); 
	пр_Общий.ИнформацияОСистемеНаСервере(ИнформацияОСистеме);
...
&НаКлиенте
Функция ПолучитьФормуПоПути(ПолныйПуть = "", ИмяОбработки = "", ИмяВнешенйФормы = "Форма", ПараметрыОбработки = Неопределено, ВладелецФормы = Неопределено, Уникальность = Неопределено, 
...
&НаКлиенте
Процедура УстановкаКаталогаФайлаНачалоВыбора(Форма, ИмяРеквизита, ВыборФайла = Истина, КаталогПоУмолчанию = "", МножественныйВыбор = Ложь, ОписаниеОповещФормы = Неопределено)	Экспорт
	РезультатУстановкиКаталогаФайлаНачалоВыбора(Форма, ИмяРеквизита, ВыборФайла, КаталогПоУмолчанию, МножественныйВыбор, ОписаниеОповещФормы);	
КонецПроцедуры 

&НаКлиенте
Функция РезультатУстановкиКаталогаФайлаНачалоВыбора(Форма, ИмяРеквизита, ВыборФайла = Истина, КаталогПоУмолчанию = "", МножественныйВыбор = Ложь, ОписаниеОповещФормы = Неопределено)	Экспорт
	
	Попытка
		
		Каталог = ВыборКаталогаФайлаНачалоВыбора(ВыборФайла, КаталогПоУмолчанию, МножественныйВыбор, Форма, ИмяРеквизита, ОписаниеОповещФормы);
...
&НаКлиенте
Функция ВыборКаталогаФайлаНачалоВыбора(ВыборФайла = Истина, КаталогПоУмолчанию = "", МножественныйВыбор = Ложь, Форма = Неопределено, ИмяРеквизита = "", ОписаниеОповещФормы = Неопределено)	Экспорт
	...
	ИнформацияОСистеме = пр_НастройкиКлиентПовтИсп.ИнформацияОСистеме();
	Если ИнформацияОСистеме.РежимСовместимости >= 830  Тогда
		ВыборКаталогаФайлаНачалоВыбораАсинх(ДиалогОткрытия, Форма, ИмяРеквизита, ОписаниеОповещФормы);
	Иначе
...
&НаКлиенте
Асинх Функция ВыборКаталогаФайлаНачалоВыбораАсинх(ДиалогОткрытия, Форма, ИмяРеквизита, ОписаниеОповещФормы = Неопределено)	Экспорт
	
	Результат = Ждать ДиалогОткрытия.ВыбратьАсинх();
	Если ЗначениеЗаполнено(Результат) Тогда
		ИмяФайла = Результат[0];     
		Если ОписаниеОповещФормы = Неопределено Тогда
			Форма[ИмяРеквизита] = ИмяФайла;
		Иначе	
			ВыполнитьОбработкуОповещения(ОписаниеОповещФормы, ИмяФайла)
		КонецЕсли;
	КонецЕсли;
